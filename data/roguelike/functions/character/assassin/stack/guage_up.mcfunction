...
execute as @s[scores={as_stack_guage=..0},tag=!ready,nbt={SelectedItem:{tag:{sub_wp:1b}}},tag=!baldo] run function roguelike:character/assassin/stack/stack_up
...
execute as @s[scores={as_stack_guage=..0},tag=!ready,nbt={SelectedItem:{tag:{sub_wp:1b}}}] run tag @s add baldo
